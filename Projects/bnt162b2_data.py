...
bnt162b2 = """GAGAAΨAAAC ΨAGΨAΨΨCΨΨ CΨGGΨCCCCA CAGACΨCAGA GAGAACCCGC   50
...
bnt162b2
len(bnt162b2)
#-- replace pseudo-U with T
bnt162b2_dna = re.sub("Ψ","T",bnt162b2)
bnt162b2_dna
...
#-- Seq Object
pzr = Seq.Seq(bnt162b2_dna)
pzr_str = pzr.__str__()
#-- SeqAA Object
pzr_aa = pzr.translate(to_stop=True)
#-- String of AAs
pzraa_str = pzr_aa.__str__()
pzraa_str
#-- ID of Start Codon
met_loc = pzraa_str.find('M')
#-- Length of translation
len(pzraa_str[met_loc:])

def str_to_codons(inp_str):
   list_ = re.findall('\w{3}?',inp_str)
   return list_

#-- Codons List
cod = str_to_codons (pzr_str)
len(cod)
...
#   aa string = pzraa_str
import pandas as pd
aL = [x for x in pzraa_str]
data = {a:c for (a,c) in map(aL,cod)}
len(data)

import itertools
schedule_raw = [x for x in "WOOOWWWOWWOWOOOOOOWOWW"]

myiter = [(len(list(y)),x) for x, y in itertools.groupby(schedule_raw)]
for x,y in myiter:
   print(str(x) + " -" + str(y)*x)
